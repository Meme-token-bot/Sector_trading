...
src/db.py                     SQLite: sentiment, weekly recaps, signal_snapshots, theme news
src/price_store.py            SQLite OHLCV cache (prices.db) + split-detection guard
src/nlp_pipeline.py           Newsletter → NewsletterAnalysis → DB
src/content_extractor.py      HTML→text, whitelisted link/PDF extraction
src/gmail_client.py           Gmail REST API (OAuth 2.0) client
src/market_engine.py          yfinance prices, FRED series, macro ratios
src/macro_alignment.py        Per-sector macro tailwind/headwind scoring (15 indicators)
src/regime_snapshot.py        Regime + breadth/dispersion snapshot (Decision Cockpit)
src/regime_analysis.py        Regime classification, per-regime stats, drawdown attribution
src/signals.py                 Pure-function convergence decision matrix (7 states + conviction)
src/signal_history.py          Weekly snapshot replay, state-change detection, forward-perf
                               tracking (rolling hit-rate, conviction calibration)
src/edge_metrics.py           Information coefficient (signal-quality by horizon)
src/expression_signals.py     Per-expression self-check + theme-news overlay
src/ticker_news.py            Google News RSS + batched OpenAI scoring per theme
src/trend.py                   Sentiment trend reconstruction
src/weekly_recap.py           Gather context + one-shot OpenAI synthesis
src/risk_metrics.py           Correlation / concentration / historical VaR-ES
src/backtest.py                Mechanical-core backtest engine
src/backtest_report.py        Regenerates BACKTEST_REPORT.md from a live run
src/walk_forward.py           Walk-forward 1D parameter sweep + joint validation
src/preflight_checks.py       Pre-trade readiness checks (shared by CLI + Dashboard)
src/tiger_client.py           Tiger positions + sector-aware drift
src/consolidation.py          ATR/Bollinger consolidation + breakout detection
src/money_flow.py             Chaikin Money Flow, OBV trend slope
src/breakout_signals.py       Unified breakout/trend/money-flow signal (Breakout scanner)
src/breakout_backtest.py      Weekly backtest harness for the breakout signal
src/charts.py, indicators.py, ui_tokens.py   Chart builders, TA indicators, shared UI tokens
app.py                         Streamlit dashboard (11 tabs)
scripts/                       CLI helpers — see below
data/sentiment.db              SQLite: newsletters + signal_snapshots + theme_news (auto-created)
data/prices.db                 SQLite OHLCV cache (auto-created)
data/walk_forward_status.json  Last walk-forward verdict (Dashboard trust badge)
data/macro_history*.csv        Backfilled full-history macro series (for research scripts)
tests/                         Pytest — 24+ files, ~250 tests, no network
```

Root-level reference docs regenerated by the scripts below, not
hand-edited: `BACKTEST_REPORT.md`, `WALK_FORWARD_REPORT.md`. Many code
comments cite `TRADING_EDGE_AUDIT.md item Bx/Sx/Ax` — the running
audit/roadmap doc behind a lot of these design decisions.

## CLI scripts (`scripts/`)

All need the project root on `PYTHONPATH`.

**Data maintenance**
- `update_prices.py` — incremental OHLCV refresh for the sector +
  expression universe (idempotent; 5y cold start on first run; does
  **not** cover the expanded crypto/metals universe — see Breakouts above)
- `extend_history.py` — wipe-and-repull a ticker further back (e.g. to
  capture 2018/2020 in the backtest)
- `backfill_macro.py`, `backfill_macro_full.py` — full-history FRED/
  yfinance macro series for the research scripts below
- `gmail_oauth_setup.py` — one-time OAuth authorization for the Inbox tab
- `backfill_inbox.py` — historical newsletter backfill by date range
- `fetch_inbox.py`, `ingest_newsletter.py` — CLI equivalents of the
  Inbox / Ingest Newsletter tabs

**Signals & reporting**
- `run_signals.py` — replicate the live signal pipeline outside
  Streamlit; writes a `signal_snapshots` row
- `refresh_weekly.py` — simpler print-only weekly signal + macro summary
- `run_backtest_report.py` — regenerate `BACKTEST_REPORT.md`
- `run_regime_analysis.py` — regime + drawdown-attribution report
- `preflight.py` — the terminal version of the Pre-Trade Checklist

**Validation & research**
- `run_walk_forward.py` — the parameter sweep behind
  `WALK_FORWARD_REPORT.md` and the Dashboard's walk-forward badge
- `walk_forward_regime.py`, `compare_regime_aware.py` — out-of-sample
  test of a *not-yet-live* regime-aware bull overlay
  (`BacktestConfig.regime_aware`) that promotes CHASE to full weight and
  drops the cash buffer in confirmed strong-bull tape
- `sector_allocator.py`, `allocator_walk_forward.py` — an alternative
  continuous cross-sectional allocator (weights every sector every
  rebalance instead of discrete BUY/HOLD/SELL), tested walk-forward
  against both the rotation baseline and 100% SPY
- `tilt_backtest.py`, `tilt_walk_forward.py` — an alternative "SPY core
  + momentum tilt that rides winners" strategy, also tested walk-forward
  — the explicit counter-experiment to the rotation model's weekly
  trim-the-winner behaviour

None of the four research scripts above are wired into the live model —
they're standing experiments testing whether a different rotation
discipline would beat both SPY and the current defensive baseline. Read
their module docstrings and walk-forward output before promoting any of
them.

## Design notes

- **Caching**: most fetched/derived data (prices, macro/FRED, signal
  history, backtest runs) is cached anywhere from 5 minutes to several
  hours depending on how fast it changes; Tiger snapshot 5min, sentiment
  aggregate 10min, weekly recap 12h in-memory + permanent on disk.
  Override any of it with *Force refresh all caches*.
...
- **Idempotency**: newsletters keyed by SHA256(content + author + date);
  Gmail messages also keyed by Gmail message ID; theme-news scores keyed
  by `(theme_key, as_of)`; signal snapshots keyed by `(as_of, ticker)`.
- **Pure signal logic**: `src/signals.py`, `src/macro_alignment.py`,
  `src/signal_history.py`, `src/expression_signals.py`,
  `src/regime_snapshot.py`, `src/regime_analysis.py`,
  `src/risk_metrics.py`, `src/edge_metrics.py`, `src/consolidation.py`,
  `src/money_flow.py`, and `src/breakout_signals.py` have no IO, no
  globals, no caching. Same inputs always give the same outputs —
...
- **Lazy Tiger import**: `tigeropen` is imported inside functions only,
  ...
  orders-panel sizing degrade gracefully to a manual-NLV entry).
...
- **State classification** (refined signal): the raw BUY / HOLD / SELL
  is enriched with weekly history + extension data into one of NEW_BUY /
  HOLD_IF_LONG / CHASE / REDUCE / WATCH / HOLD / SELL, then a macro
  veto/override pass can demote a fresh BUY, demote a stale BUY further,
  or promote a lagging HOLD to WATCH. Thresholds in `config/settings.py`
  (`extension_pct_cutoff`, `stale_buy_weeks`, `history_weeks`,
  `strong_rs_margin`, `macro_strong_count`, `chase_weight_fraction`).
- **CHASE is not fully excluded by default.** `chase_weight_fraction`
  defaults to `0.25` (positive out-of-sample lift in the walk-forward
  sweep) — a CHASE sector gets a partial position sized as a fraction of
  the confirmed per-name weight, funded from the cash buffer, never
  diluting NEW_BUY / HOLD_IF_LONG sizing. Set it to `0` to go back to
  full exclusion.
- **The Breakout scanner is independent by design**: it never touches
  `build_signals`, `refine_signals`, `target_weights`, or
  `signal_snapshots` — it exists specifically because those functions
  can never emit BUY without sentiment coverage, and most of the
  expanded universe has none.
- **No leveraged ETFs** in the sector-model expression lists — plain
  equity only, operating leverage from underlying business cost
  structures, not derivatives or daily resets. (The expanded/Breakout
  universe does include *unleveraged* spot crypto and crypto-equity
  exposure, which the sector model never touches.)